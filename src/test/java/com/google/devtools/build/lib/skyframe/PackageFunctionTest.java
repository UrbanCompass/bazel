...
public class PackageFunctionTest extends BuildViewTestCase {
  ...
  public void testInconsistentNewPackage() throws Exception {
    scratch.file("pkg/UCBUILD", "subinclude('//foo:sub')");
  ...
  public void testInconsistentMissingPackage() throws Exception {
    ...
    scratch.file("/root1/pkg/UCBUILD", "subinclude('//foo:sub')");

    Path root2 = fs.getPath("/root2");
    scratch.file("/root2/foo/UCBUILD");
    ...
    scratch.file("/root1/foo/UCBUILD");
  ...
  public void testPropagatesFilesystemInconsistencies() throws Exception {
    ...
    Path fooBuildFile = scratch.file("foo/UCBUILD");
    Path fooDir = fooBuildFile.getParentDirectory();

    // Our custom filesystem says "foo/UCBUILD" exists but its parent "foo" is a file.
    ...
    String expectedMessage = "/workspace/foo/UCBUILD exists but its parent path /workspace/foo isn't "
  ...
  public void testGlobOrderStable() throws Exception {
    scratch.file("foo/UCBUILD", "sh_library(name = 'foo', srcs = glob(['**/*.txt']))");
  ...
  public void testGlobOrderStableWithLegacyAndSkyframeComponents() throws Exception {
    ...
    scratch.overwriteFile(
        "foo/UCBUILD", "sh_library(name = 'foo', srcs = glob(['*.txt', '*.config']))");
    getSkyframeExecutor()
        .invalidateFilesUnderPathForTesting(
            reporter,
            ModifiedFileSet.builder().modify(new PathFragment("foo/UCBUILD")).build(),
            rootDirectory);
    value = validPackage(skyKey);
    assertThat(
            (Iterable<Label>)
        ...
        .inOrder();
    scratch.overwriteFile(
        "foo/UCBUILD", "sh_library(name = 'foo', srcs = glob(['*.txt', '*.config'])) # comment");
    getSkyframeExecutor()
        .invalidateFilesUnderPathForTesting(
            reporter,
            ModifiedFileSet.builder().modify(new PathFragment("foo/UCBUILD")).build(),
  ...
  public void testIOErrorLookingForSubpackageForLabelIsHandled() throws Exception {
    ...
    Path barBuildFile = scratch.file("foo/bar/UCBUILD");
  ...
  public void testLoadRelativePath() throws Exception {
    scratch.file("pkg/UCBUILD", "load('ext', 'a')");
  ...
  public void testIncrementalSkyframeHybridGlobbingOnDanglingSymlink() throws Exception {
    Path packageDirPath = scratch.file("foo/UCBUILD",
    ...
    getSkyframeExecutor().invalidateFilesUnderPathForTesting(reporter,
        ModifiedFileSet.builder().modify(new PathFragment("foo/UCBUILD")).build(), rootDirectory);
