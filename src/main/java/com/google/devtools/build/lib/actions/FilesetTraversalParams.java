...
public interface FilesetTraversalParams {
  ...
  /** Desired behavior if the traversal hits a directory with a UCBUILD file, i.e. a subpackage. */
  public enum PackageBoundaryMode {
    ...
    // and lock down the semantics of FilesetEntry.srcdir to only accept other Filesets or UCBUILD
   ...
   *
   * <ul>
   * <li>The root of package traversals is the package directory, i.e. the parent of the UCBUILD file.
  ...
  interface DirectTraversal {
     ...
     * These are usually whole-package traversals, i.e. when FilesetEntry.srcdir refers to a UCBUILD
