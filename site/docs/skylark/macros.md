...
A macro is a function called from the UCBUILD file that can instantiate rules.
...
If you need to know the package name (i.e. which UCBUILD file is calling the
...
* `bazel query --output=build //my/path:all` will show you how the UCBUILD
...
Explain clearly to the user what went wrong and how to fix their UCBUILD file. It
...
* In UCBUILD files, the `name` argument of the macros must be a keyword
...
The UCBUILD file will become simply:
...
In order to keep UCBUILD files clean and declarative, you must put the function in
